...
def get_csv():
    ...
    try:
        ...
        # Build file name
        file_name = (
            f"CpinData_{_date}.csv" if len(_date) > 0
            else "CpinData_All.csv")
...
def handle_name():
    ...
    try:
        return json.dumps(config.config_data['cpin_data_collector']['name'])
...
def main():
    ...
    # Print version
    logging.info(f"Starting Cpin Data Collector server version {version.get_version()}")
