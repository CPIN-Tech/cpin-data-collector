...
def update_data(value, year, month, day, hour):
    '''Updates data in the data base.'''
    try:
...
def main():
    ...
    while run:
        ...
        try:
            ...
            update_data(data[-1]['produced_self'], data[-1]['date'].split('-')[0], data[-1]['date'].split('-')[1], data[-1]['date'].split('-')[2], data[-1]['date'].split('-')[3])
        except Exception:
